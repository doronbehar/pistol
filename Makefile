...
ASSETS_UPLOAD_TARGETS=$(foreach attr, $(NIX_ATTRIBUTES), releaseAssetsUploaded/$(attr))
...
# https://stackoverflow.com/a/64797833/4935114
.SECONDEXPANSION:
$(ASSETS_UPLOAD_TARGETS): $$(filter %$$(@F),$(NIX_TARGETS))
	@mkdir -p releaseAssetsUploaded
	@gh release upload v$(VERSION) "$@"
	@touch "$@"

release: pistol.1 newTag
	@gh release create v$INPUT_STRING --generate-notes pistol.1
	$(MAKE) $(ASSETS_UPLOAD_TARGETS)
	$(MAKE) cleanReleaseTemps

cleanReleaseTemps:
	rm -f newTag newVersionFile $(NIX_TARGETS) $(ASSETS_UPLOAD_TARGETS)
endif
